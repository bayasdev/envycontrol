...
XSETUP_NVIDIA_XRANDR_SCRIPT_PATH = '/etc/X11/xdm/Xsetup'

XSETUP_NVIDIA_XRANDR_SCRIPT = '''
# Automatically generated by EnvyControl - Start
xrandr --setprovideroutputsource {} NVIDIA-0
xrandr --auto
# Automatically generated by EnvyControl - End
'''

NVIDIA_INITRAMFS_MODULES_PATH = '/etc/initramfs-tools/modules'

NVIDIA_INITRAMFS_MODULES_CONTENT = '''
# Automatically generated by EnvyControl - Start
nvidia
nvidia_drm
nvidia_modeset
...
def _switcher(mode, display_manager = ''):
    ...
    if mode == 'integrated':
        ...
        try:
            ...
            if enable_load_early:
                _add_nvidia_modules_to_initramfs()
        except Exception as e:
            print(f'Error: {e}')
            sys.exit(1)
        _rebuild_initramfs()
...
def _cleanup():
    ...
    # remove xrandr commands from XDM Xsetup file
    _remove_envycontrol_block_from_file(XSETUP_NVIDIA_XRANDR_SCRIPT_PATH)

    # remove nvidia modules from initramfs
    _remove_nvidia_modules_from_initramfs()

def _remove_envycontrol_block_from_file(path):
    with open(path, "r+") as f:
        ...
        for line in new_f:
            if "# Automatically generated by EnvyControl - Start" in line:
                envycontrol_block = 1
            elif "# Automatically generated by EnvyControl - End" in line:
                envycontrol_block = 0
            elif envycontrol_block == 0:
                f.write(line)
        f.truncate()
...
def _check_display_manager():
    ...
    pattern = re.compile(r'(ExecStart=.*\/usr\/bin\/|\/usr\/sbin\/)(gdm|sddm|lightdm|xdm)')
    try:
        with open('/etc/systemd/system/display-manager.service',mode='r', encoding='utf-8') as f:
            ...
            # print(f'Detected display manager: {display_manager}')
    except Exception:
        display_manager = ''
...
def _setup_display_manager(display_manager):
    ...
    if display_manager == 'sddm':
        ...
        if igpu_vendor == "amd":
            ...
            _append_to_file(XSETUP_NVIDIA_XRANDR_SCRIPT_PATH, XSETUP_NVIDIA_XRANDR_SCRIPT.format("modesetting"))
...
def _add_nvidia_modules_to_initramfs():
    ...
    # RHEL and SUSE derivatives
        ...
        # TODO - unsure how to do 
        
def _remove_nvidia_modules_from_initramfs():
    # Debian and Ubuntu derivatives
    if os.path.exists('/etc/debian_version'):
        _remove_envycontrol_block_from_file(NVIDIA_INITRAMFS_MODULES_PATH)
    # RHEL and SUSE derivatives
    #elif os.path.exists('/etc/redhat-release') or os.path.exists('/usr/bin/zypper'):
...
def main():
    ...
    parser.add_argument('--dm', type=str, metavar='DISPLAY_MANAGER', action='store',
                        help='Manually specify your Display Manager. This is required only for systems without systemd. Supported DMs: gdm, sddm, lightdm, xdm')
