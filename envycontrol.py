...
TEARING_FIX_PATH = '/etc/X11/xorg.conf.d/10-nvidia-comp.conf'

TEARING_FIX_CONTENT = '''# Automatically generated by EnvyControl

Section "OutputClass"
    Identifier "nvidia anti-tearing"
...
def _switcher(mode):
    ...
    if mode == 'integrated':
        ...
        try:
            ...
            if click.confirm('Do you want to enable ForceCompositionPipeline?', default=True):
                _create_file(TEARING_FIX_PATH, TEARING_FIX_CONTENT)
        except Exception as e:
            print(f'Error: {e}')
            sys.exit(1)
        _rebuild_initramfs()
...
def _cleanup():
    ...
    to_remove = (BLACKLIST_PATH,UDEV_PATH, XORG_PATH, TEARING_FIX_PATH, '/etc/X11/xorg.conf.d/90-nvidia.conf', MODESET_PATH, '/etc/lightdm/nvidia.sh', '/etc/lightdm/lightdm.conf.d/20-nvidia.conf')
